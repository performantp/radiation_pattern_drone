...
void Si4735::begin(byte options, byte bus_arg){
   ...
   if( !(options & BEGIN_DO_NOT_INIT_BUS) ){
      Wire.begin();
      Wire.setClock(400000);        //JCR (800K)FAST BUT MAY FAIL ; (400K) OK; (100K) STD
   }
...
void Si4735::setMode(byte new_mode, byte options, byte audio_mode){
   ...
   if(new_mode != RADIO_OFF){
      ...
      if(new_mode==FM){
         arg1 |= POWER_UP_ARG1_FUNC_FM;  //POWER_UP_ARG1_FUNC_FM;
      }else{  //AM, SW, LW
         arg1 |= POWER_UP_ARG1_FUNC_AM;  //POWER_UP_ARG1_FUNC_AM;
      }
      _buffer[1] = arg1;
...
word Si4735::valid_status(byte arg){
   //Set TUNE_STATUS command
   if(_mode==FM){
      _buffer[0]=CMD_FM_TUNE_STATUS;
   }else{  //AM, SW, LW
   ...
   //Convert frequency high and low bytes into word and save. Then return current frequency.
   byte valid = _buffer[1] & FIELD_RSQ_STATUS_RESP2_SEEKABLE;
   return valid;
...
//JCR - get valid tune bit
word Si4735::getValid(bool clearSTC){
   return valid_status(clearSTC ? TUNE_STATUS_CLEAR_STC : 0);
}
